...
**Onyx pin:** `2f5bab9946539cbe083010c9ae6fbc59b47ae377`, sparse reference at `C:/tmp/onyx` (**ONYX** below).
...
Onyx keeps `BodyComponent`/`OrganComponent` in the bare `Content.Shared.Body` namespace; Wolfgate keeps them in `Content.Shared.Body.Components` / `.Organ`. Nine wound files carry `using Content.Shared.Body;`, but only four actually need a symbol from it and get a `// WOLFGATE` using swap: `WoundSystem.cs:34` (`BodyComponent`), `OrganDamageSystem.cs:87` (`OrganComponent`), `WoundDamageProjectionSystem.cs:29` (`InitialBodySystem`), `FractureEffectsSystem.cs:34-35` (`OrganGot*Event`). In the other five — `WoundPrototype.cs:3`, `WoundStatusEffectSystem.cs:1`, `BodyPartFunctionalitySystem.cs:1`, `WoundBleedingSystem.cs:3`, `FractureAlertSystem.cs:2` — the using **resolves and is unused**: C# declares every enclosing namespace, so `Content.Shared.Body` exists as the parent of `Content.Shared.Body.Part`. **Keep them verbatim.** Do not "clean" them; each removal is a spurious diff at re-sync time.
...
| **GUARD E2** | same | `:274` | `!HasComp<WoundHostComponent>(ent) &&` in front of the `GetBloodLevelPercentage(...) < BloodlossThreshold` pale-message test. **Land this only together with the `_Onyx/HealthExaminable` port**, or the "looks pale" message silently disappears with nothing replacing it. | WP10 |
