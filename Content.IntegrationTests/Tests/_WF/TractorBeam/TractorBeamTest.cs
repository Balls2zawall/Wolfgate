...
public sealed class TractorBeamTest
{
    ...
    public async Task PinCountersQueuedThrustAndTorqueChargesPowerAndSlipsDuringBrownout()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            var thrust = new Vector2(-beam.MaxForce * 0.1f, beam.MaxForce * 0.1f);
            ...
            Assert.That(predictedTargetVelocity.Length(), Is.LessThan(0.0001f));
            Assert.That(MathF.Abs(predictedTargetRotation), Is.LessThan(0.0001f));
            Assert.That(power.DrawRate, Is.GreaterThan(beam.HoldingPower + (beam.MaxPower - beam.HoldingPower) * 0.3f));
            Assert.That(beam.Strain, Is.GreaterThan(0.3f));
            ...
            Assert.That(predictedTargetVelocity.Length(), Is.GreaterThan(0f), "Pinning cannot retain authority without strain power.");
